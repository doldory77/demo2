...
async function viewMember(params, res) {
    ...
    let member = await query('sys_member', 'selectMember', newParams)
...
router.post('/member_id_chk', (req, res) => {
    (async function(){
        ...
        let user = await query('sys_member', 'selectMember', {id})
        if (!user || !user[0]) {
...
router.post('/member_add', (req, res) => {
    (async function(){
        ...
        let result = await query('sys_member', 'insertMember', newParams)
        console.log(result)
...
async function viewMemberDetail(params, res) {
    let mem = await query('sys_member', 'selectMember', {seq_no:params.seq_no})
    let jikbun = await query('sys_code', 'selectCodeByParentCd', {parent_cd:'0300'})
    let mwgubun = await query('sys_code', 'selectCodeByParentCd', {parent_cd:'0900'})
    let contact_kind = await query('sys_code', 'selectCodeByParentCd', {parent_cd:'0400'})
    let contact = await query('sys_member', 'selectContactByMbSeqNo', {mb_seq_no:params.seq_no})
    let addr = await query('sys_member', 'selectAddrByMbSeqNo', {mb_seq_no:params.seq_no})
    let picture = await query('sys_member', 'selectFile', {src_tbl_nm:'member', rf_key:params.seq_no})
